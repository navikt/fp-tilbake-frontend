...
envsubst '$APP_URL $APP_PORT $APP_HOSTNAME $APP_NAME $APP_VERSION $APP_PATH_PREFIX $APP_CALLBACK_PATH' < /etc/nginx/conf.d/app.conf.template > /etc/nginx/conf.d/default.conf
